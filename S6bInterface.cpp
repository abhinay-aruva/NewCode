...
void S6BInterface::printStats(std::string &node)
{
    ...
    while(it != res.end())
    {
        ...
        while(it1 != tmp->end())
        {
           ...
           if (reqIt != req.end())
           {
               ...
               {
                   s6bStats.latency[(it->first)-1] = ((RTTCount * s6bStats.latency[(it->first)-1]) + (it1->second)-(reqIt1->second))/(++RTTCount);
                   reqTmp->erase(reqIt1);
               }
               else
               {
                   s6bStats.timeoutCount[(it->first)-1]++;
               }                
           }
           else
           {
    ...
    while(reqIt != req.end())
    {
        ...
        while(reqIt1 != reqTmp->end())
        {
            ...
            {
                s6bStats.timeoutCount[(reqIt->first)-1]++;
                reqTmp->erase(reqIt1++);
            }
            else
            {
